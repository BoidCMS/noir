<?php defined( 'App' ) or die( 'BoidCMS' ); global $App ?>
<link rel="canonical" href="<?= $App->url( ( $App->page === 'home' && ! $App->get( 'blog' ) ) ? '' : $App->page ) ?>">
<link rel="preload" as="font" href="<?= $App->theme( 'poppins_latin.woff2', false ) ?>" type="font/woff2" crossorigin>
<link rel="preload" as="font" href="<?= $App->theme( 'poppins_latin_ext.woff2', false ) ?>" type="font/woff2" crossorigin>
<link rel="preload stylesheet" as="style" href="https://cdn.jsdelivr.net/npm/sysacss@0.1.0/sysa.min.css">
<link rel="preload stylesheet" as="style" href="<?= $App->theme( 'style.css', false ) ?>">
<?= $App->get_filter( '<link rel="shortcut icon" type="image/svg+xml" href="data:image/svg+xml;base64,">', 'favicon' ) ?>
